...
public class MybChartSUMMARY extends MybChart{
		...
		try {
			...
			if(totalCount>0){
				 aggregation = newAggregation(match(Criteria.where("questionnaireId").is(questionnaireId)),
						 match(ct),unwind("$answers"),match(Criteria.where("answers.questionId").is("1")),
			...
			for(int i=(joTrends.size()-1);i>=0;i--){
				...
				if(historyTotalCount>0){			
					 aggregation = newAggregation(match(Criteria.where("questionnaireId").is(questionnaireId)),
							 match(ct),
							 match(ca),
							 unwind("$answers"),match(Criteria.where("answers.questionId").is("1")),
							 match(Criteria.where("answers.optionValue").in("8","9","10")),group("$answers.questionName").count().as("count"));
					 results = mongoTemplate.aggregate(aggregation, "answer", TagCount.class);
					 tagCount = results.getMappedResults();				 
					if(tagCount.size()>0) {
			...
			chartLegend.append("<span style='float: left;font-size: 12px;font-weight: bold;color:#6f6f6f'>总体满意度</span><span class='"
					+ alertColor
					+ "'></span><br>")
			...
			StringBuffer option =new StringBuffer("{title : {text: '',subtext: '',x:'center'},")
					...
					.append("series : [{name:'总体满意度',type:'pie',radius : ['67%', '70%'],startAngle:90,clockWise:true,avoidLabelOverlap: false,color: ['#78973e', '#eaeaea'],center: ['45%', '50%'],")
					...
					.append("[{value:"
							...
							+ ", name:'满意比例'},{value:"
							+ (100-satifiedPct)
							+ ", name:'非满意比例',itemStyle:{  normal : {label : {show : false},labelLine : {show : false}}}}]")
					.append("}]}");
			...
			StringBuffer optionTrend =new StringBuffer("{title : {text: '',subtext: '',x:'center'},")
					...
					.append(legendStr)
					.append("},")
					.append("series : [{name:'总体满意度',type:'bar',itemStyle:{normal:{color:'#78973e'}},")
					.append("data: ")
