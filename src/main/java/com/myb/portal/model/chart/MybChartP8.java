...
public class MybChartP8 extends MybChart{
	...
	public AjaxReq getData(MongoTemplate mongoTemplate) {
		...
		try {
			...
			if("chartCustDimnsn".equals(dimensionType)){
				...
				JSONObject jbDimn = JSONObject.fromObject(dimension);
				jaMembers =  jbDimn.getJSONArray("members");	
				for(JSONObject jot:jaMembers){
					names.add(jot.getString("name"));
					memberIds.add(jot.getString("memberId"));
				}
			}else if("chartMultiDimnsn".equals(dimensionType)){
				dimensionStr = "";
				JSONObject jbDimn = JSONObject.fromObject(dimension);
				qstId = jbDimn.getString("questionId");
				qstName = jbDimn.getString("questionName");				
				List<JSONObject> jbOpt = jbDimn.getJSONArray("options");				
				...
				for(JSONObject jot:jbOpt){
					names.add(jot.getString("optionValue"));
				}
			}else{
			...
			if(fdmp.get("filterOnly") != null){
				...
				for(JSONObject member:jaMembers){
					ct = new Criteria().andOperator(fdmp.get(member.get("memberId")).toArray(new Criteria[fdmp.get(member.get("memberId")).size()]));
					cts.add(ct);
					 aggregation = newAggregation(match(Criteria.where("questionnaireId").is(questionnaireId)),match(ct),
