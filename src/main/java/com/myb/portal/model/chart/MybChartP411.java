...
public class MybChartP411 extends MybChart{
	...
	public AjaxReq getData(MongoTemplate mongoTemplate) {
		...
		try {
			 ...
			 * 
			 * Comments:
			 * NPS（Net Promoter Score，净推荐率）是反映市场口碑的重要指标。
			 * 1、从本次数据来看，推荐意愿较强的顾客（8-10分）占比xx%，xx%的顾客推荐意愿较弱（1-5分），NPS指数为：XX。
			 * 2、与行业水平相比，xx餐厅的NPS<插入判别描述>
			 * 
			 */
			
			if(count > 0){
				Double satifiedPct =  new Double((satisfied/count)*100);
				Double npsPct =  new Double(((satisfied-oneOrfive)/count)*100);
				Double normPct = new Double(65);//TO-DO
				String starVal = "20";
				...
				if(npsPct >=90){
					starVal = "100";
				}else if(npsPct >= 75 && npsPct < 90){
					starVal = "80";
				}else if(npsPct >= 60 && npsPct < 75){
					starVal = "50";
				}else if(npsPct >= 40 && npsPct < 60){
					starVal = "40";
				}else if(npsPct < 40){
					starVal = "20";
				}
				...
				}else if((npsPct - normPct) >= 3){
					gapDesc = "略微领先于行业平均水平";
					alertColor = "alertgreen";
				}else if((npsPct - normPct) > -3){
					gapDesc = "属于行业一般水平";
					alertColor = "alertyellow";				
				}else if((npsPct - normPct) > -5){
					gapDesc = "略微落后于行业平均水平";
					alertColor = "alertred";
				}else{
					gapDesc = "显著落后于行业平均水平";
			...
			StringBuffer comments = new StringBuffer("NPS（Net Promoter Score，净推荐率）是反映市场口碑的重要指标。<br />");
			comments.append("1、从本次数据来看，推荐意愿较强的顾客（8-10分）占比<span class=\"satisfied\">")
			.append(satifiedPct)
			.append("%</span>, <span class=\"unsatisfied\">"
					+ (oneOrfive/count)*100
					+"的顾客推荐意愿较弱（1-5分），NPS指数为：<span class=\"neutral\">"
					+npsPct
					+ "</span>.<br />"
					+ "2、与行业水平相比，<span class=\"store\">"
					+ custName
					+ "</span>的NPS<span class=\"comment-desc\">"
					+ gapDesc
					+ "</span>.");

			StringBuffer chartLegend = new StringBuffer("<div style=\"line-height:20px\">");			
			chartLegend.append("<div class=\"legendtitle\">NPS</div><div class=\"legendtitlenumber satisfiedPercent\">")
			.append(npsPct)
			...
			aReq.setData(rspjb);
				...
				JSONObject rspjb = new JSONObject();
				rspjb.put("type", "pie");
				rspjb.put("title", "推荐意愿NPS在此条件下没有数据！");
				aReq.setSuccess(true);
				aReq.setData(rspjb);
			}
		} catch (Exception e){
