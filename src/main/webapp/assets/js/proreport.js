...
/**
 * TODO:
 * 1. Save Filters: Time, Store - _saveFilters
 * 2. Save Customize Dimension
 * 3. Upload changes to server
 * 4. Retrieve restults from server
 */
...
reportGlobal.P52={};
reportGlobal.P52.labelTop = {normal : {label : {show : true,position : 'center',formatter : '{b}',textStyle: {baseline : 'bottom'}},labelLine : {show : false}}};
reportGlobal.P52.labelFromatter = {normal : {label : {formatter : function (params){return 100 - params.value + '%'},textStyle: {baseline : 'top'}}}};
reportGlobal.P52.labelBottom = {normal : {color: '#ccc',label : {show : true,position : 'center'},labelLine : {show : false}},emphasis: {color: 'rgba(0,0,0,0)'}};
reportGlobal.P52.tooltip={trigger: 'item',formatter: '{a} <br/>{b} : {c} ({d}%)'};
/*
函数：格式化日期
参数：formatStr-格式化字符串
d：将日显示为不带前导零的数字，如1
...
function startInits(){
    ...
    $('input[name="start"]').datepicker({ maxDate: 0 });
    $('input[name="end"]').datepicker({ maxDate: 0 });
	_initSampleFilter();
	
	$('.chart-main').height($('.middle-center.ui-layout-pane.ui-layout-pane-center').height()*0.9);
	$('.chart-main').width($('.middle-center.ui-layout-pane.ui-layout-pane-center').width()*0.9);
}
...
function _initSampleFilter(){
	...
	var value = {};
	var period = {start_time:cdate.format('yyyy/MM/dd'),end_time:''};
	var store = {store_type:"",store_id:[""],store_labels:[""]};
	reportGlobal.samplefilter ={};
...
function _initDimnDialog(){
	...
	if(lisize > 2){
      ...
      console.log($($('.newsmplbtn').parent().parent().find('li')[i]).text());
...
function dimn_remove_samplegroup(dom){
	...
	$(dom).parent().parent().find('li:last').height($(dom).parent().parent().find('li:last').height() + 35);
...
function newsamplegroup(dom){
	...
	if($('.dimension-dialog .store').html() !=''){
		_saveDimnsnFilters($(dom).parent().parent().find("li[class='active']").find('span').attr('id'),$(dom).parent().parent().find("li[class='active']").find('span').text(),false);
	}
	$(dom).parent().parent().find("li").removeClass("active");
	//Add new member tab
	var rn = uuid(30,16);
	$(dom).parent().before(' <li><span onclick="_leftDimnGroupClick(this);" class="dimnSGtextspan" id="SG'+rn+'" >自定义样品组0'+($('.dimn-left-nav').find('ul li').length-1)+'</span> <button onclick="dimn_remove_samplegroup(this);" type="button" class="close">×</button></li>');
...
function _redrawdimnsn4tabs(){
	...
	$('.dimension-dialog input[name="end"]').datepicker( "option", "disabled", false );
...
function _initDimnTempData(){
	...
	}else if(reportGlobal.dimensionstemp.dimensionId != $('.dimension-name input').attr('id')){
		reportGlobal.dimensionstemp.dimensionId = $('.dimension-name input').attr('id');
 ...
 * 				<li><input name="dimnsncheck" type="checkbox" value="time" />就餐时间</li>
				...
				com.myb.questiontype.Degree, com.myb.questiontype.Judge, com.myb.questiontype.MultiSelect
 * */
...
function _loadSavedDimnsnList(jsondata){
	...
	if(reportGlobal.selectedStoreType == 'store'){
		...
		$('.dimensions-dropdown-div-left').children().first().parent().html('<li><input type="checkbox" value="store" name="dimnsncheck">门店</li>');
	}else if(reportGlobal.selectedStoreType == 'storegroup'){
		$('.dimensions-dropdown-div-left').children().first().show();
		$('.dimensions-dropdown-div-left').children().first().parent().html('<li><input type="checkbox" value="storegroup" name="dimnsncheck">门店组</li>');
...
function _loadSavedDimnsnDialog(mdata){
	...
	//restore the left nav
	for(var i=0;i<reportGlobal.dimensionstemp["members"].length;i++){
		$('.newsmplbtn').parent().before(' <li><span onclick="_leftDimnGroupClick(this);" class="dimnSGtextspan" id="'
			+reportGlobal.dimensionstemp["members"][i]["memberId"]+'" >'+reportGlobal.dimensionstemp["members"][i]["name"]+'</span> <button onclick="dimn_remove_samplegroup(this);" type="button" class="close">×</button></li>');
		//$(dom).parent().prev().addClass("active");
		$('.newsmplbtn').parent().next().height($('.newsmplbtn').parent().next().height() - 35);

		$('#'+reportGlobal.dimensionstemp["members"][i]["memberId"]).dblclick(function(){
		    var span = $(this);
		    // 根据文本创建文本框 并加入表表中--文本框的样式自己调整
		    var text = span.text();
		    var txt = $("<input type='text' class='dimnSGinput'>").val(text);
...
function _loadSelectedOptions(so){
	...
	buildDimensionDialog(reportGlobal.jsondata);
	$('.dimension-dialog input[name="start"]').datepicker('setDate',new Date(sf.period.start_time));
	$('.dimension-dialog input[name="end"]').datepicker('setDate',new Date(sf.period.end_time));
	if(!sf.period.end_time){
		$('.dimension-dialog input[name="end"]').datepicker( "option", "disabled", true );
		$('.dimension-dialog input[name="nowtime"]').prop('checked',true);
	}
	$('input[name="storeradio"][value="'+sf.store.store_type+'"]').prop('checked',true);
	for(var i=0; i<sf.store.store_id.length;i++){
		$('#'+sf.store.store_type+'dialog input[id="'+sf.store.store_id[i]+'"]').prop('checked',true);
	}
...
function _saveDimnsnFilters(id, name, saveflag){
	...
	var start_time = $('.dimension-dialog input[name="start"]').datepicker('getDate');
	var end_time = $('.dimension-dialog input[name="end"]').datepicker('getDate');
	var period = {
			"start_time":start_time == null ? '':start_time.format('yyyy/MM/dd'),
            "end_time":end_time == null ? '':end_time.format('yyyy/MM/dd')
	};
	//To save the store option and selected stores of the filter.
	var store_id = [];
	var store_labels = [];
	$('#'+$('input[name="storeradio"]:checked').val()+'dialog input:checked').each(function(){
	  store_id.push($(this).attr('id'));
	  store_labels.push($(this).val());
	})
	var store = {
			"store_type":$('.dimension-dialog input[name="storeradio"]:checked').val(),
			"store_id":store_id,
		...
		if(reportGlobal.jsondata["dimensions"]){
			for(var i=0;i<reportGlobal.jsondata["dimensions"].length;i++){
				if(reportGlobal.jsondata["dimensions"][i].dimensionId == dmnsnid){
					reportGlobal.jsondata["dimensions"].splice(i,1,reportGlobal.dimensionstemp);
					findmatchedflag = true;
			...
			if(!findmatchedflag){
				reportGlobal.jsondata["dimensions"].push(reportGlobal.dimensionstemp);
			}
...
function buildLeftNav(jsondata) {
	var qGroup  = jsondata['questionGroup'].sort(function (a, b) {return a.sortNumber - b.sortNumber;});
	var leftNav = jsondata['questions'].sort(function (a, b) {return a.sortNumber - b.sortNumber;});
	var flag = false;
	var html = "";
	...
	for (var i = 0; i < qGroup.length; i++){
		if(qGroup[i]['businessType']!='groupCustomerVoice'){
			...
			html += "<h3><a "
					+ " question_group_id='" + qGroup[i]['questionGroupId'] + "'"
			...
			for( var j = 0; j<leftNav.length; j ++){				
				if(leftNav[j]['questionGroupId'] == qGroup[i]['questionGroupId'] &&   (leftNav[j]['activePeriod']=='' || (leftNav[j]['activePeriod']!='' && _compareDates(leftNav[j]['activePeriod'], ds2)))){
					html += "<li><a "
					+ " questionId='" + leftNav[j]['questionId'] + "'"
...
function _leftNavQuestionClick(dom) {
	$(dom).parent().parent().parent().parent().find("ul li").removeClass("active");
	$(dom).parent().addClass("active");
...
function _compareDates(ds1,ds2){
  ...
  for(var i=0;i<ds1.length; i++){
	  ...
	  ds1[i].endTime == null||ds1[i].endTime == ''?d1e = new Date('2100/01/01'):d1e = new Date(ds1[i].endTime);
	  ds2.end_time == null?d2e = new Date('2100/01/01'):d2e = new Date(ds2.end_time);
	  _minD(d1e,d2e).format('yyyy/MM/dd') >=_maxD(d1s,d2s).format('yyyy/MM/dd')?result=true:result=false;
	  if(result){
		  return result;
	  }
  }
...
function buildFilterTopNav(jsondata){
	//<li><div onclick="_topFilterClick(this);" title="筛选条件:男；北京/上海/广州；18-45岁"><span class="filtertextspan">未保存</span></div></li>
...
function buildDimensionNav(jsondata){
	var topNav = jsondata['savedDimnTab'].replace(/"/g,'').replace('[','').replace(']','').replace(/ /g,'').split(',');
...
function _topFilterClick(dom){
	//TO-DO: 1. whether load filters in the sample dialog? 2. load the page according to the saved filter
...
function _topNavOtherClick(dom) {
	...
	var qdata = reportGlobal.jsondata['questions'].sort(function (a, b) {return a.sortNumber - b.sortNumber;});
	for ( var i = 0; i < qdata.length; i++) {
...
function _topNavCustDimnClick(dom) {
	...
	if(reportGlobal.jsondata["dimensions"]){
		for(var i=0;i<reportGlobal.jsondata["dimensions"].length;i++){
			if(reportGlobal.jsondata["dimensions"][i].dimensionId == $(dom).attr('id')){
...
/**
 * William
 * Function example: convert 1 to '01'
 * **/
...
function _buildSampleDialog(jsondata) {
	...
	for ( var i = 0; i < storedata.length; i++) {
		htmltemp += "<input type='checkbox' onclick='_filterchecked(this)' name='门店' value='"+storedata[i]['store_name'] +"' id='" + storedata[i]['store_id']
				+ "'><label for='" + storedata[i]['store_id'] + "'>"
				+ storedata[i]['store_name'] + "</label>";
		if (i%2 > 0){
			htmltemp +=  "<br>";
		}
	}
	...
	for ( var i = 0; i < storegroupdata.length; i++) {
		htmltemp += "<input type='checkbox' onclick='_filterchecked(this)' name='门店组' value='"+storegroupdata[i]['store_group_name'] +"' id='" + storegroupdata[i]['store_group_id']
				+ "'><label for='" + storegroupdata[i]['store_group_id'] + "'>"
	...
	var qGroup  = jsondata['questionGroup'].sort(function (a, b) {return a.sortNumber - b.sortNumber;});
	var qdata = jsondata['questions'].sort(function (a, b) {return a.sortNumber - b.sortNumber;});
	var v = 0;
	var ds1;
	var ds2 = {
			start_time:$('.sample-dialog input[name="start"]').datepicker('getDate'),
	...
	for (var j = 0; j<qGroup.length; j++){
		if(qGroup[j]['businessType'] !='groupCustomerVoice' && qGroup[j]['filterFlag']){
			...
			html += "<div class='heading' id='"+qGroup[j]['questionGroupId']+"' >" + qGroup[j]['name']
					+ "</div>";
			var judgeCount = 0;
			for( var i = 0; i < qdata.length; i++){
				if(qdata[i]['filterFlag'] && qGroup[j]['questionGroupId'] == qdata[i]['questionGroupId']&& (qdata[i]['activePeriod']=='' || (qdata[i]['activePeriod']!='' && _compareDates(qdata[i]['activePeriod'],ds2)))){
					if (qdata[i]['questionType'] == 'com.myb.questiontype.SingleSelect') {
						...
						var option = qdata[i]['options'].sort(function (a, b) {return a.sortNumber - b.sortNumber;});
						for ( var k = 0; k < option.length; k++) {
							if(option[k]['activeFlag']){
								html += "<input type='checkbox' onclick='_filterchecked(this)' name='"+qdata[i]['questionName']
										+ "' value='"
										+  option[k]['optionValue']
										+"' id='" + option[k]['optionId']
										+ "'><label for='"
										+ option[k]['optionId'] + "'>"
						...
						html += "<input type='checkbox' onclick='_filterchecked(this)' name='"+qdata[i]['questionName']
								...
								+ "'><label for='" + qdata[i]['questionId'] + "'>"
								+ qdata[i]['questionName'] + "</label>";						
						if(judgeCount ==1){
							html += "</span>"
						}
					} else if (qdata[i]['questionType'] == 'com.myb.questiontype.Degree'){
						html += "<div class='heading1' id='"+qdata[i]['questionId']+"' >" + qdata[i]['questionName']
...
function buildDimensionDialog(jsondata ) {
	...
	var ds1;
	var ds2 = {
			start_time:$('.dimension-dialog input[name="start"]').datepicker('getDate'),
			end_time:$('.dimension-dialog input[name="end"]').datepicker('getDate')
	};
	...
	for (var j = 0; j<qGroup.length; j++){
		...
		for( var i = 0; i < qdata.length; i++){
		    //TO-DO
			//if(!qdata[i]['activePeriod'] || (qdata[i]['activePeriod'] && _compareDates(qdata[i]['activePeriod'],ds2))){
			if(qGroup[j]['questionGroupId'] == qdata[i]['questionGroupId'] && (qdata[i]['activePeriod']=='' || (qdata[i]['activePeriod']!='' && _compareDates(qdata[i]['activePeriod'],ds2)))){
				if (qdata[i]['questionType'] == 'com.myb.questiontype.SingleSelect') {
					html += "<div class='heading1' id='"+qdata[i]['questionId']+"' ><div class='questionlabel'>" + qdata[i]['questionName'] + "</div><div class='optionsdiv'>";
					var option = qdata[i]['options'].sort(function (a, b) {return a.sortNumber - b.sortNumber;});
					for ( var k = 0; k < option.length; k++) {
...
function _updateTips( t ) {
	  var tips = $( ".validateTips" );
	      tips
	        .text( t )
	        .addClass( "ui-state-highlight" ).show();
	      setTimeout(function() {
...
function _saveFilters(name, saveflag){
	//To save the time period of the filter.
	var start_time = $('.sample-dialog input[name="start"]').datepicker('getDate');
...
function _filterchecked(dom){
	if(dom.checked){
		...
		if(dom.value == "store" || dom.value == "storegroup"){
		    ...
		    dialog.dialog( "open" );
		    $( "#"+dom.value+"dialog" ).parent().css({left:'300px',top:'140px'});
...
//	});
	...
	//Cancel the saving of the customized dimension and close the dimension dialog
	...
	$(".savedimnsnbtn").click(function() {
		$(".dimension-dialog").toggle();
		_saveDimnsnFilters($('.dimn-sg-name').find('li[class="active"]').find('span').attr('id'),$('.dimn-sg-name').find('li[class="active"]').find('span').text(),true);
...
function bindButtons() {
	...
	$(".dimn-confirm").click(function() {
		...
		for(var i =0; i< ckdmns.length; i++){
			if($(ckdmns[i]).parent().text()=='门店' || $(ckdmns[i]).parent().text()=='门店组'){
				funcname = '_topNavStoreClick';
				type = "stor";
			}else if($(ckdmns[i]).parent().parent().attr('class') == 'dimensions-dropdown-div-right'){
				funcname = '_topNavCustDimnClick';
				type = "dimn";
			}

			html+='<li><div onclick="'+funcname+'(this);" id="'+$(ckdmns[i]).val()+'">'+$(ckdmns[i]).parent().text().replace('编辑删除','')+' </div></li>'
			savedDimn ={};
			savedDimn.id = $(ckdmns[i]).val();
			savedDimn.type = type;
			savedDimn.name = $(ckdmns[i]).parent().text().replace('编辑删除','');
	...
	$("#showmenu").selectmenu({change: function( event, ui ) {
		if(ui.item.label =='年'){
...
/*
	...
	var v ;
	// start testing data setting
	
	if ($(".dimnsn>li>div:eq(0)").hasClass("active")) {
		v = "location";
	}else if ($(".dimnsn>li>div:eq(1)").hasClass("active")) {
		v = "mealtime";
	}else if ($(".dimnsn>li>div:eq(2)").hasClass("active")) {
		v = "custtype";
	}else if ($(".dimnsn>li>div:eq(3)").hasClass("active")) {
		v = "whowith";
	}else if ($(".dimnsn>li>div:eq(4)").hasClass("active")) {
		v = "age";
	}else if ($(".dimnsn>li>div:eq(5)").hasClass("active")) {
		v = "sex";
	}            
 * */
...
function loadPage() {
	...
	var postdata = {questionnaire_id:reportGlobal.id,question_id:qid,
			page:page,filter:JSON.stringify(reportGlobal.samplefilter),dimensiontype:reportGlobal.dimensionType,dimension:JSON.stringify(reportGlobal.dimensions),questionGroup:JSON.stringify(qstGrup),groupId:qid,questionName:questionName};
	$.ajax({
        type: 'post',
...
function getTotalCount(){
	var cdate = new Date();
	cdate.setMonth(cdate.getMonth() -6);
	var start_time = _string2Json(reportGlobal.samplefilter.period).start_time == ''?cdate.format('yyyy/MM/dd'):_string2Json(reportGlobal.samplefilter.period).start_time;
	var end_time = _string2Json(reportGlobal.samplefilter.period).end_time ==''?(new Date()).format('yyyy/MM/dd'):_string2Json(reportGlobal.samplefilter.period).end_time;
	$('.timeperiodlabel').html(start_time +' - '  + end_time );
	var postdata = {questionnaire_id:reportGlobal.id,filter:JSON.stringify(reportGlobal.samplefilter)};
	$.ajax({
        type: 'post',
	    ...
	    cache: false,
	    success: function (data) {			
			$('.filterlabel').html('样本量: '+(_string2Json(data.data)).totalCount);
	    },
	    error: function(XMLHttpRequest, textStatus, errorThrown) {
	    	console.log(XMLHttpRequest.status);
	    	console.log(XMLHttpRequest.readyState);
