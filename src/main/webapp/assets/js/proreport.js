...
function startInits() {
  ...
  $('.chart-main').height($(
    '.middle-center.ui-layout-pane.ui-layout-pane-center').height() * 0.9);
  $('.chart-main').width($(
    '.middle-center.ui-layout-pane.ui-layout-pane-center').width() * 0.9);
}
...
function dimn_remove_samplegroup(dom) {
  ...
  $(dom).parent().parent().find('li:last').height($(dom).parent().parent().find(
    'li:last').height() + 35);
...
function newsamplegroup(dom) {
  ...
  if ($('.dimension-dialog .store').html() != '') {
    _saveDimnsnFilters($(dom).parent().parent().find("li[class='active']").find(
      'span').attr('id'), $(dom).parent().parent().find(
...
function _initDimnTempData() {
  if (!reportGlobal.dimensionstemp) {
    ...
    reportGlobal.dimensionstemp.dimensionId = $('.dimension-name input').attr(
      'id');
  ...
  for (var i = 0; i < qdata.length; i++) {
    ...
    if (qdata[i]['activeFlag'] && qdata[i]['questionType'] ==
      ...
      html += "<li><input name='dimnsncheck' type='checkbox' value='" + qdata[i]
        ['questionId'] + "' />" + qdata[i]['questionName'] + "</li>";
    }
  }
  if (reportGlobal.selectedStoreType == 'store') {
    $('.dimensions-dropdown-div-left').children().first().show();
    $('.dimensions-dropdown-div-left').children().first().parent().html(
...
function _loadSavedDimnsnDialog(mdata) {
  ...
  for (var i = 0; i < reportGlobal.dimensionstemp["members"].length; i++) {
    ...
    //$(dom).parent().prev().addClass("active");
    $('.newsmplbtn').parent().next().height($('.newsmplbtn').parent().next().height() -
      35);

    $('#' + reportGlobal.dimensionstemp["members"][i]["memberId"]).dblclick(
      function() {
        var span = $(this);
        // 根据文本创建文本框 并加入表表中--文本框的样式自己调整
        var text = span.text();
...
function _loadSelectedOptions(so) {
  ...
  buildDimensionDialog(reportGlobal.jsondata);
  $('.dimension-dialog input[name="start"]').datepicker('setDate', new Date(sf.period
    .start_time));
  $('.dimension-dialog input[name="end"]').datepicker('setDate', new Date(sf.period
    .end_time));
  ...
  for (var i = 0; i < sf.store.store_id.length; i++) {
    $('#' + sf.store.store_type + 'dialog input[id="' + sf.store.store_id[i] +
      '"]').prop('checked', true);
  }
  ...
  for (var i = 0; i < qGroup.length; i++) {
    if (qGroup[i]['businessType'] != 'groupCustomerVoice') {
      ...
      html += "<h3><a " + " question_group_id='" + qGroup[i]['questionGroupId'] +
        "'" + " href='#' onclick='_leftNavQuestionGroupClick(this);'>" + qGroup[
          i]['name'] + "</a></h3>";
      ...
      for (var j = 0; j < leftNav.length; j++) {
        if (leftNav[j]['questionGroupId'] == qGroup[i]['questionGroupId'] && (
            leftNav[j]['activePeriod'] == '' || (leftNav[j]['activePeriod'] !=
              '' && _compareDates(leftNav[j]['activePeriod'], ds2)))) {
...
function _compareDates(ds1, ds2) {
  ...
  for (var i = 0; i < ds1.length; i++) {
    ...
    _minD(d1e, d2e).format('yyyy/MM/dd') >= _maxD(d1s, d2s).format('yyyy/MM/dd') ?
...
function buildTempFilterTopNav() {
  ...
  if (topNav) {
    ...
    html +=
      "' ><span class='filtertextspan'  onclick='_topFilterClick(this);'>" +
      fName +
...
function buildDimensionNav(jsondata) {
  var topNav = jsondata['savedDimnTab'].replace(/"/g, '').replace('[', '').replace(
    ']', '').replace(/ /g, '').split(',');
...
function _topNavCustDimnClick(dom) {
  ...
  if (reportGlobal.jsondata["dimensions"]) {
    for (var i = 0; i < reportGlobal.jsondata["dimensions"].length; i++) {
      if (reportGlobal.jsondata["dimensions"][i].dimensionId == $(dom).attr(
          'id')) {
        if (reportGlobal.dimensions.dateType != null && reportGlobal.dimensions
          .dateType != '') {
          var dt = reportGlobal.dimensions.dateType;
...
function _buildSampleDialog(jsondata) {
  ...
  for (var i = 0; i < storedata.length; i++) {
    htmltemp +=
      ...
      storedata[i]['store_name'] + "' id='" + storedata[i]['store_id'] +
      "'><label for='" + storedata[i]['store_id'] + "'>" + storedata[i][
        'store_name'
      ] + "</label>";
    if (i % 2 > 0) {
      htmltemp += "<br>";
  ...
  for (var i = 0; i < storegroupdata.length; i++) {
    htmltemp +=
      ...
      storegroupdata[i]['store_group_name'] + "' id='" + storegroupdata[i][
        'store_group_id'
    ...
    if (qGroup[j]['businessType'] != 'groupCustomerVoice' && qGroup[j][
      ...
      for (var i = 0; i < qdata.length; i++) {
        if (qdata[i]['filterFlag'] && qGroup[j]['questionGroupId'] == qdata[i][
            ...
            '' && _compareDates(qdata[i]['activePeriod'], ds2)))) {
          if (qdata[i]['questionType'] == 'com.myb.questiontype.SingleSelect') {
            ...
            for (var k = 0; k < option.length; k++) {
              if (option[k]['activeFlag']) {
                html +=
                  "<input type='checkbox' onclick='_filterchecked(this)' name='" +
                  qdata[i]['questionName'] + "' value='" + option[k][
                    'optionValue'
                  ] + "' id='" + option[k]['optionId'] + "'><label for='" +
                  option[k]['optionId'] + "'>" + option[k]['optionValue'] +
                  "</label>";
              }
            }
            html += "</div></div>";
...
function buildDimensionDialog(jsondata) {
  ...
  for (var j = 0; j < qGroup.length; j++) {
    ...
    for (var i = 0; i < qdata.length; i++) {
      ...
      if (qGroup[j]['questionGroupId'] == qdata[i]['questionGroupId'] && (qdata[
          i]['activePeriod'] == '' || (qdata[i]['activePeriod'] != '' &&
          _compareDates(qdata[i]['activePeriod'], ds2)))) {
        if (qdata[i]['questionType'] == 'com.myb.questiontype.SingleSelect') {
          html += "<div class='heading1' id='" + qdata[i]['questionId'] +
            "' ><div class='questionlabel'>" + qdata[i]['questionName'] +
          ...
          for (var k = 0; k < option.length; k++) {
            if (option[k]['activeFlag']) {
              html +=
                "<input type='checkbox' onclick='_filterchecked(this)' name='" +
                qdata[i]['questionName'] + "' value='" + option[k]['optionId'] +
                "' id='" + option[k]['optionId'] + "'><label for='" + option[k]
                ['optionId'] + "'>" + option[k]['optionValue'] + "</label>";
            }
          }
          html += "</div></div>";
        } else if (qdata[i]['questionType'] == 'com.myb.questiontype.Judge') {
...
function buildDimensionDialogButtons(jsondata) {
  ...
  $(".savedimnsnbtn").click(function() {
    $(".dimension-dialog").toggle();
    _saveDimnsnFilters($('.dimn-sg-name').find('li[class="active"]').find(
        'span').attr('id'), $('.dimn-sg-name').find('li[class="active"]')
      .find('span').text(), true);
...
function bindButtons() {
  ...
  $(".dimn-confirm").click(function() {
    ...
    for (var i = 0; i < ckdmns.length; i++) {
      if ($(ckdmns[i]).parent().text() == '门店' || $(ckdmns[i]).parent().text() ==
        ...
        funcname = '_topNavStoreClick';
        type = "stor";
      } else if ($(ckdmns[i]).parent().parent().attr('class') ==
        'dimensions-dropdown-div-right') {
        funcname = '_topNavCustDimnClick';
        type = "dimn";
      }

      html += '<li><div onclick="' + funcname + '(this);" id="' + $(ckdmns[
        i]).val() + '">' + $(ckdmns[i]).parent().text().replace('编辑删除',
        '') + ' </div></li>'
      savedDimn = {};
  ...
  $("#showmenu").selectmenu({
    change: function(event, ui) {
      ...
      if ($(".dim.time").html() != "" || $(".dim.time_dimnsn").html() !=
...
function loadPage() {
  ...
  reportGlobal.specialQuestions == '' ? reportGlobal.specialQuestions = JSON.stringify(
    qlist) : 1 == 1;
...
function getTotalCount() {
  ...
  var start_time = _string2Json(reportGlobal.samplefilter.period).start_time ==
    '' ? cdate.format('yyyy/MM/dd') : _string2Json(reportGlobal.samplefilter.period)
    .start_time;
  var end_time = _string2Json(reportGlobal.samplefilter.period).end_time == '' ?
    (new Date()).format('yyyy/MM/dd') : _string2Json(reportGlobal.samplefilter.period)
    .end_time;
...
function drawCharts(jsondata) {
  ...
  if (jsondata.option) {
    chart.setOption(typeof(jsondata.option) == 'string' ? _string2Json(jsondata
