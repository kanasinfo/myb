...
<div id="body">
	...
	<c:if test="${list !=null}">
		...
		<c:if test="${group.key.activeFlag==1}">
			<c:if test="${group.key.type==1&&!group.key.filterFlag&&group.key.selectQuestionCount!=0}">
				<div class="row ask">
			...
			<c:if test="${group.key.type==0&&!group.key.filterFlag&&group.key.selectQuestionCount!=0}">
				<c:forEach var="question" items="${group.value}">
					<c:if test="${question.activeFlag ==true }">
					<div class="row ask">
        			 ...
        			 for (var i = 0; i <dataDecode.answers.length; i++) {
 						if(dataDecode.answers[i].questionnaireId == id){
 							if($("#" +id).is(':checked')){
 								...
 								dataDecode.answers[i][key] = 1;
 							}else{
 								dataDecode.answers[i].questionIdValue=0;
 	 							dataDecode.answers[i].optionValue = 0;
 	 							var key = id+"_optionValue";
 								dataDecode.answers[i][key] = 0;
 							}
 						}
 					}
        		}
